...
let move (input : status) : unit =
  ...
  let y = current_y () in
  match key with
  | 'w' -> Graphics.moveto x (y + 10)
  (* fill_circle x (y + 10) 3 *)
  | 'a' -> Graphics.moveto (x - 10) y
  (* fill_circle (x - 10) y 3 *)
  | 's' -> Graphics.moveto x (y - 10)
  (* fill_circle x (y - 10) 3 *)
  | 'd' -> Graphics.moveto (x + 10) y
  (* fill_circle (x - 10) y 3 *)
  | _ -> ()

let init (input : unit) =
  Graphics.open_graph " 1000x1000+500+500";
  Graphics.set_window_title "hai :3";
  (* Graphics.draw_rect 25 25 25 25; *)
